...
class Jadwal extends CI_Controller {
	...
	public function tambahjadwal(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$tujuan = $this->db->query("SELECT * FROM varis WHERE kd_varis ='".$this->input->post('tujuan')."'")->row_array();

			if ($asal == $tujuan['kd_varis']) {
				$this->session->set_flashdata('message', 'swal("Başarısız", "Sefer Hedefleri Aynı Olamaz", "error");');
				...
				$this->session->set_flashdata('message', 'swal("Başarılı", "Yeni sefer eklenmiştir", "success");');
				redirect('backend/jadwal');
			}
		}
	}
	...
	public function viewjadwal($id=''){
		$data['title'] = "Varış Listesi";
	 	...
	 	if ($sqlcek) {
	 		...
	 		$data['jadwal'] = $sqlcek;
			$data['title'] = "Seferi Görüntüle";
			$this->load->view('backend/view_jadwal',$data);
	 	} else {
	 		$this->session->set_flashdata('message', 'swal("Başarısız", "Bir şeyler yanlış gitti. Lütfen tekrar deneyin", "error");');
			redirect('backend/jadwal');
	 	}
	}	
}
